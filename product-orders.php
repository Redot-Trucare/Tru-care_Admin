...
echo '<script type="text/javascript">
           window.location = "product-orders.php"
      </script>';
...
$itemsql2 = "INSERT INTO order_action(order_id, order_date, order_sign, order_status, login_id, update_on) VALUES ('" . $ordrow['id'] . "','" . $ordrow['date'] . "','Order Placement','Confirm the Order','" . $_SESSION['username'] . "','$date')";
//mysql_query($funcsql);
...
$emsql = $db->query("SELECT * FROM customers WHERE id = " . $loginrow['customer_id']);
...
$ordrow = $ordsql->fetch_assoc();
$date = date("Y-m-d H:i:s");  
$itemsql2 = "INSERT INTO order_action(order_id, order_date, order_sign, order_status, login_id, update_on) VALUES ('" . $ordrow['id'] . "','" . $ordrow['date'] . "','Order Placement','Cancel the Order','" . $_SESSION['username'] . "','$date')";
//mysql_query($funcsql);
mysqli_query($db, $itemsql2);
$loginsql = $db->query("SELECT * FROM logins WHERE id = " . $ordrow['customer_id']);
//mysqli_query($db,$loginsql);
...
$loginrow = $loginsql->fetch_assoc();
$emsql = $db->query("SELECT * FROM customers WHERE id = " . $loginsql['customer_id']);
$emrow = $emsql->fetch_assoc();
/*
ini_set("SMTP","mail.gmail.com");
    ini_set("smtp_port","25");
...
echo "<br><br><br><br>";*/
$orderssql = $db->query("SELECT * FROM orders WHERE status = 2 ORDER BY id Asc");
...
echo "</td>";
echo "<td><a href='product-orders.php?func=conf&id=" . $row['id']. "'><i class='fa fa-pencil'></i> Confirm</a></td>";
echo "<td><a href='product-orders.php?cancel=conf&id=" . $row['id']. "'><i class='fa fa-trash'></i> Cancel</a></td>";
