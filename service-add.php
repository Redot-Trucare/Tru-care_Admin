                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="page-title-box">
                                ...
                                <h4 class="page-title">Add Serices</h4>
                            </div>
                        </div>
                    </div>
                    <!-- end page title -->
                                ...
                                <div class="">
                                    <form action="" method="post" enctype="multipart/form-data">
                                        ...
                                        <div class="form-group mb-4">
                                            ...
                                            <input type="file" name="file2" class="form-control" data-height="210" />
                                        </div>
                                        <div class="form-group mb-4">
      ...
      $query = $db->query("SELECT title FROM services WHERE title='$title'");
        ...
        if($count == 1) { 
			  $statusMsg = "<br><div class='alert alert-danger mb-0 text-center' role='alert'><strong>Already Exists</strong></div>";
                ...
                $insert = $db->query("INSERT INTO services (title, description, status, uploaded_on) VALUES ('".$title."','".$description."','".$status."', NOW())"); 
                if($insert){ 
    ...
    $ext = strtolower(pathinfo($name, PATHINFO_EXTENSION));
        ...
        if(in_array($ext, $permissible_extension)){
            if(move_uploaded_file($temp,"services/".$name)){
                $sql1 = $db->query("Update services Set image = '$name' Where title='$title'");
			}
		}
     }
    ...
    $ext = strtolower(pathinfo($name, PATHINFO_EXTENSION));
        ...
        if(in_array($ext, $permissible_extension)){
            if(move_uploaded_file($temp,"services-thumbnail/".$name)){
                $sql1 = $db->query("Update services Set thumb_nail = '$name' Where title='$title'");
			}
		}
     }  
} 
